...
function openSchoolDetail(index) {
  ...
  const application = verifiedParent
    ? `<section style="border-top:1px solid var(--border-color);padding-top:14px;"><h3 style="margin:0 0 5px;">Apply to this school</h3><p style="margin:0 0 12px;color:var(--text-muted);font-size:.84rem;">Your verified Little Feet account is required. The application is sent directly to this school’s principal when its school account is active. Do not include medical or other sensitive details here.</p><form onsubmit="submitSchoolApplication(event, ${index})" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px;"><label>Parent / guardian name<input name="guardianName" autocomplete="name" required value="${escapeWorkspaceText(currentUser.name || '')}"></label><label>Contact email<input name="contactEmail" type="email" autocomplete="email" required value="${escapeWorkspaceText(currentUser.username || '')}"></label><label>Contact phone<input name="contactPhone" autocomplete="tel" required inputmode="tel"></label><label>Learner name<input name="learnerName" autocomplete="off" required></label><label>Date of birth<input name="dateOfBirth" type="date" autocomplete="off" required></label><label>Age group / intended grade<input name="gradeOrAgeGroup" autocomplete="off" required placeholder="e.g. Grade R / Toddler"></label><label>Intended start date<input name="intendedStart" type="date" autocomplete="off" required></label><label>Home area / suburb<input name="homeArea" autocomplete="address-level2" required></label><label style="grid-column:1/-1;">Application note<textarea name="notes" required rows="3" placeholder="Why you are applying, preferred contact time, and any non-sensitive information the school should know."></textarea></label><label for="schoolApplicationConsent" style="grid-column:1/-1;display:flex;gap:8px;align-items:flex-start;"><input id="schoolApplicationConsent" name="applicationConsent" type="checkbox" required> I confirm these details are accurate and I am authorised to apply for this learner.</label><button class="submit-btn" style="grid-column:1/-1;">Send application to principal</button></form></section>`
