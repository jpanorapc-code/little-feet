...
assert.match(page, /--lf-bioluminescent-teal:#00B4D8/);
assert.match(page, /--panel-bg:#1B4965/);
assert.match(page, /body:not\(\.light-mode\)[\s\S]*--primary-color:#0D0D1D/);
assert.match(page, /body:not\(\.light-mode\)[\s\S]*--panel-bg:#0D0D1D/);
assert.match(page, /linear-gradient\(180deg, #1A1A2D, #0D0D1D\)/);
assert.match(page, /body:not\(\.light-mode\) \.app-sidebar[\s\S]*rgba\(17,17,33,\.96\)[\s\S]*rgba\(8,8,18,\.97\)/);
assert.match(page, /body:not\(\.light-mode\) \.portal-welcome-banner[\s\S]*rgba\(13,13,29,\.985\)[\s\S]*admin-mascot-dashboard-4k\.jpg/);
...
assert.match(page, /body\.light-mode[\s\S]*--primary-color:#00B4D8/);
assert.match(page, /body\.light-mode \.guide-open,[\s\S]*color:#00B4D8 !important/);
assert.match(page, /body\.light-mode input:focus,[\s\S]*border-color:#00B4D8 !important/);
...
assert.match(page, /0 0 0 1000px #0f304e inset/);
assert.match(page, /body\.light-mode #loginForm #loginUsername:-webkit-autofill/);
assert.match(page, /0 0 0 1000px #abc5cd inset/);
assert.match(page, /id="icon-volume-off"/);
assert.match(page, /data-mute-label="Sound On" data-unmute-label="Muted"/);
...
assert.match(page, /@media \(min-width:1600px\)[\s\S]*animation-duration:34s/);
assert.match(page, /@media \(max-width:900px\) and \(max-height:600px\) and \(orientation:landscape\)[\s\S]*\.login-info-runner/);
assert.match(page, /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.login-info-runner-track[\s\S]*animation:none/);
...
assert.match(page, /cinematic\.css\?v=20260926-cinematic-v14/);
...
assert.match(page, /Portal panel sizing audit/);
assert.match(page, /Professional future surface system/);
assert.match(page, /--portal-header-height/);
assert.match(page, /#dashboardSection > \.app-sidebar\.is-collapsed ~ \.container/);
assert.match(page, /#ticketsTab #ticketForm > \.flex-form-row \{[\s\S]*grid-template-columns:repeat\(auto-fit/);
assert.match(page, /@media \(min-width:1920px\) and \(min-height:900px\)/);
assert.match(page, /@media \(min-width:2560px\)/);
assert.match(client, /function observePortalHeaderSize\(\)/);
assert.match(client, /--portal-header-height/);
